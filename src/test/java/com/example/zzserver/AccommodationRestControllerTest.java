...
List<AccommodationResponseDto> accommodations = List.of(acc1, acc2, acc3, acc4);
    ...
    public void apiAccommmodationsTestPost() throws Exception {
        ...
        when(accommodationService.createAccommodation(any(AccommodationRequest.class), any())).thenReturn(newId);
        ...
        mockMvc.perform(
                        org.springframework.restdocs.mockmvc.RestDocumentationRequestBuilders.multipart("/api/accommodations/")
                                .file(imageFile)
...
//                        )
  ...
  AccommodationRequest updateRequest = AccommodationRequest.builder()
        ...
        when(accommodationService.updateAccommodations(any(UUID.class), any(AccommodationRequest.class), any(), any()))
                .thenReturn(updateRequest.getBussinessUserId());

        mockMvc.perform(
                        org.springframework.restdocs.mockmvc.RestDocumentationRequestBuilders.multipart("/api/accommodations/{id}", id)
                                .file(requestFile)
                                .contentType("multipart/form-data")
                                .with(request -> { request.setMethod("PATCH"); return request; })
                )
                .andExpect(status().isOk())
                .andExpect(content().string(objectMapper.writeValueAsString(updateRequest.getBussinessUserId())))
                .andDo(MockMvcResultHandlers.print())
                .andDo(MockMvcRestDocumentation.document("{class-name}/{method-name}",
                        preprocessRequest(prettyPrint()),
                        preprocessResponse(prettyPrint())
