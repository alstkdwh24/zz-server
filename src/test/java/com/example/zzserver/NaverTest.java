...
@Import({ MockConfig.class, JpaConfig.class, RedisConfig.class }) // ✅ TestRestTemplateConfig를 MockConfig로 변경
...
public class NaverTest {
        ...
        @MockitoBean // ✅ RestTemplate도 Mock으로 변경
                ...
                mockMvc.perform(post("/api/naver/token").param("grant_type", "authorization_code")
                                ...
                                .andDo(MockMvcRestDocumentation.document("{class-name}/{method-name}",
                                                ...
                                                responseFields(
                                                                ...
                                                                fieldWithPath("expires_in").type(JsonFieldType.STRING)
                                                                                .description("토큰 만료 시간"),
                                                                fieldWithPath("token_type").type(JsonFieldType.STRING)
                                                                                .description("토큰 타입"))));
        }
        ...
        public void apiNaverUserInfo() throws Exception {
                ...
                NaverLoginInfoDto.NaverUser user = new NaverLoginInfoDto.NaverUser(
                                "123", "홍길동", "20-29", "male", "hong@example.com", "홍길동", "01-01", "1990",
                                "010-1234-5678");
                ...
                mockMvc.perform(get("/api/naver/userInfo")
                                ...
                                .andDo(MockMvcRestDocumentation.document("{class-name}/{method-name}",
                                                ...
                                                preprocessResponse(modifyHeaders().remove("Content-length")
                                                                ...
                                                                prettyPrint()),
                                                responseFields(
                                                                fieldWithPath("resultcode").type(JsonFieldType.STRING)
                                                                                .optional().description("응답 코드"),
                                                                fieldWithPath("message").type(JsonFieldType.STRING)
                                                                                .optional().description("응답 메시지"),
                                                                fieldWithPath("response.id").type(JsonFieldType.STRING)
                                                                                .description("네이버 사용자 ID"),
                                                                fieldWithPath("response.nickname")
                                                                                .type(JsonFieldType.STRING)
                                                                                .description("네이버 사용자 닉네임"),
                                                                fieldWithPath("response.email")
                                                                                .type(JsonFieldType.STRING)
                                                                                .description("네이버 사용자 이메일"),
                                                                fieldWithPath("response.gender")
                                                                                .type(JsonFieldType.STRING)
                                                                                .description("네이버 사용자 성별"),
                                                                fieldWithPath("response.age").type(JsonFieldType.STRING)
                                                                                .description("네이버 사용자 연령대"),
                                                                fieldWithPath("response.name")
                                                                                .type(JsonFieldType.STRING).optional()
                                                                                .description("네이버 사용자 이름"),
                                                                fieldWithPath("response.birthday")
                                                                                .type(JsonFieldType.STRING).optional()
                                                                                .description("생일"),
                                                                fieldWithPath("response.birthyear")
                                                                                .type(JsonFieldType.STRING).optional()
                                                                                .description("출생년도"),
                                                                fieldWithPath("response.mobile")
                                                                                .type(JsonFieldType.STRING).optional()
                                                                                .description("휴대폰 번호"),
                                                                fieldWithPath("response.profile_image")
                                                                                .type(JsonFieldType.STRING).optional()
                                                                                .description("프로필 이미지"))));
        }
        ...
        void reissueAccessToken_실제호출_및_저장확인() {
                ...
                String expectedEmail = "test@naver.com";

                // NaverService Mock 설정
                when(naverService.reissueAccessToken(refreshToken)).thenReturn(expectedEmail);

                // RefreshTokenRedisRepository Mock 설정
                RedisRefreshToken mockToken = new RedisRefreshToken();
                mockToken.setRefreshToken(refreshToken);
                List<RedisRefreshToken> mockTokenList = Arrays.asList(mockToken);
                when(refreshTokenRedisRepository.findAll()).thenReturn(mockTokenList);
